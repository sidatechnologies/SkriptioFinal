...
function refineKeyPhrases(candidates, sentences, docTitle) { const BAN = new Set(['used','reduce','model','models','like','models like','used reduce','work','process','one process','ongoing process','study kit','synthesis','skriptio','aceel','chapter','section','unit','topic','introduction','overview','conclusion','abstract','references','bibliography','acknowledgements','contents','index']); const ok = []; const seen = new Set(); const titleL = String(docTitle||'').toLowerCase().trim(); for (const p of candidates) { const key = (p||'').toLowerCase().trim(); if (!key) continue; const tokens = key.split(' '); if (tokens.some(t => t.length < 3)) continue; if (BAN.has(key)) continue; if (key === 'data') continue; if (key.includes(' like')) continue; if (key.startsWith('used')) continue; if (titleL && (key === titleL || titleL.includes(key) || key.includes(titleL))) continue; const s = sentences.find(sen => new RegExp(`\\b${escapeRegExp(p)}\\b`, 'i').test(sen)); if (!s) continue; if (looksLikeHeadingStrong(s, docTitle)) continue; if ([...seen].some(k => k.includes(key) || key.includes(k))) continue; ok.push(p); seen.add(key); } return ok; }
...
function ensureSentence(text = '') { let t = String(text || '').trim(); if (!t) return ''; t = charCleanupLight(t); t = fixMidwordSpaces(fixSpacing(t)); if (!/[\.!?]$/.test(t)) t += '.'; t = t.charAt(0).toUpperCase() + t.slice(1); return t.trim(); }
function limitSentences(text = '', maxSent = 2) { const parts = String(text || '').split(/(?<=[\.!?])\s+/).filter(Boolean); return parts.slice(0, Math.max(1, maxSent)).join(' ').trim(); }
function normalizeToLength(s, target) { let t = ensureSentence(s); const slack = 0; if (t.length > target + slack) return cutToWordBoundary(t, target); if (t.length < target - slack) { const suffixes = [ ' It refers to details discussed in the material.', ' It connects to the described topic.', ' It is grounded in the provided text.' ]; let i = detIndex(t, suffixes.length); while (t.length < target - 2 && i < suffixes.length) { t = (t.replace(/[\.!?]+$/, '')) + suffixes[i++]; } if (t.length < target - 2) t = cutToWordBoundary(t + ' It provides context.', target); if (!/[\.!?]$/.test(t)) t += '.'; } return t; }
function harmonizeOptions(opts = [], correctIndex = 0) { const base = (opts[correctIndex] || '').length || 120; const target = Math.max(50, Math.min(220, base)); return opts.map((o) => normalizeToLength(o, target)); }
...
function finalizeOptions(type, arranged = [], correctIdx = 0) { let formatted = arranged.map((o, i) => { if (type === 'formula') return formatFormulaOption(o, i); if (type === 'cloze') return formatTermOption(o, i); return ensureSentence(limitSentences(o, 2)); }); return harmonizeOptions(formatted, correctIdx); }
...
function buildConceptQuestion(phrase, sentences, phrases, qi = 0, explain = false) { const hasPhrase = (p, s) => new RegExp(`\\b${escapeRegExp(p)}\\b`, 'i').test(s); const s = sentences.find(ss => hasPhrase(phrase, ss) && ss.length >= 50) || sentences[qi % Math.max(1, sentences.length)] || ''; const correct = summarizeSentence(s, 180); const distractPool = sentences.filter(x => x !== s).slice(0, 40).map(z => summarizeSentence(z, 160)); const opts = distinctFillOptions(correct, distractPool, [], [], 4); const placed = placeDeterministically(opts, correct, qi % 4); { const formatted = finalizeOptions('concept', placed.arranged, placed.idx); return { id: `c-${qi}-${detIndex(phrase)}`, type: 'concept', question: `Which statement best describes "${phrase}"?`, options: formatted, answer_index: placed.idx, explanation: explain ? `The correct statement mentions "${phrase}" in context.` : '' }; } }

function buildFormulaQuestion(formula, formulas, qi = 0, explain = false) { const others = (formulas || []).filter(f => f !== formula).slice(0, 6); const opts = distinctFillOptions(formula, others, [], [], 4); const placed = placeDeterministically(opts, formula, (qi + 1) % 4); const formatted = finalizeOptions('formula', placed.arranged, placed.idx);
  const correctIdx = placed.idx;
  return { id: `f-${qi}-${detIndex(formula)}`, type: 'formula', question: `Which formula appears in the material?`, options: formatted, answer_index: correctIdx, explanation: explain ? 'This exact formula string was detected in the text/PDF.' : '' }; }
...
function buildQuiz(text, phrases, formulas, opts = {}) {
  ...
  let si = 0; while (out.length < total && si < sentences.length) { const s = sentences[si++]; const correct = summarizeSentence(s, 160); const distract = sentences.filter(x => x !== s).slice(0, 6).map(z => summarizeSentence(z, 150)); const opts2 = distinctFillOptions(correct, distract, [], phrases, 4); const placed = placeDeterministically(opts2, correct, (out.length + 1) % 4); const formatted = finalizeOptions('statement', placed.arranged, placed.idx); out.push({ id: `s-${out.length}-${detIndex(s)}`, type: 'statement', question: `Which statement is supported by the material?`, options: formatted, answer_index: placed.idx, explanation: explain ? 'This statement is derived from the provided content.' : '' }); }
