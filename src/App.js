    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <Section id="features" title="Features" subtitle="Everything you need to turn notes into learning outcomes.">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-card border border-border rounded-lg p-5">
            <div className="flex items-center gap-2 text-sm font-medium">Combine sources</div>
            <p className="text-sm text-foreground/80 mt-1">Upload PDFs and paste notes together for richer, more varied quizzes.</p>
          </div>
          <div className="bg-card border border-border rounded-lg p-5">
            <div className="flex items-center gap-2 text-sm font-medium">Private by design</div>
            <p className="text-sm text-foreground/80 mt-1">Runs 100% in your browser after load. No servers, no data sharing.</p>
          </div>
          <div className="bg-card border border-border rounded-lg p-5">
            <div className="flex items-center gap-2 text-sm font-medium">Fast &amp; responsive</div>
...
export function Studio() {
    ...
    try {
      ...
      if (token) {
        ...
        if (quiz) {
          ...
          setAnswers({}); setScore(null); setEvaluated(false);
  ...
  const ensureJsPDF = async () => {
    const maybe = await getJsPDF(1200);
  ...
  const addHeader = (doc) => {
    ...
    try { /* centered logo reserved spot; no-op to keep spacing */ } catch {}
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setResult(kit);
      setAnswers({}); setScore(null); setEvaluated(false);
      setLoadingStep('Building theory');
  ...
  const evaluate = () => {
    ...
    let correct = 0;
    result.quiz.forEach((q, i) => { if (answers[i] === q.answer_index) correct++; });
  ...
  const compactQuiz = (quiz, title) => {
    ...
    const q = quiz.map(qq => [ indexOf(qq.question), qq.options.map(o => indexOf(o)), qq.answer_index ]);
  ...
  const robustCopy = async (str) => {
    try { if (navigator.clipboard && window.isSecureContext) { await navigator.clipboard.writeText(str); return true; } } catch {}
    try { const ta = document.createElement('textarea'); ta.value = str; ta.style.position = 'fixed'; ta.style.left = '-9999px'; document.body.appendChild(ta); ta.focus(); ta.select(); const ok = document.execCommand('copy'); document.body.removeChild(ta); if (ok) return true; } catch {}
    try { window.prompt('Copy this link', str); return true; } catch {}
    return false;
  };
  const buildShareURL = () => {
  ...
  const shareAnswers = async () => {
    ...
    try { if (navigator.share) { await navigator.share({ title: result.title || 'Skriptio Quiz', text: 'Review my quiz answers', url }); } else { const ok = await robustCopy(url); if (ok) toast({ title: 'Share link copied', description: 'Paste it in chat or email.' }); else toast({ title: 'Copy failed', description: 'Please copy manually from the prompt.' }); } } catch {}
  };
  const copyShareURL = async () => {
    if (!result?.quiz?.length) return; const url = buildShareURL(); const ok = await robustCopy(url); if (ok) toast({ title: 'Share link copied', description: 'Paste it anywhere.' }); else toast({ title: 'Copy failed', description: 'Please copy manually from the prompt.' });
  };
      ...
      <main className="max-w-6xl mx-auto px-6 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex flex-wrap items-center gap-2">
            <Button variant="outline" disabled={!result?.quiz?.length || pdfBusy.quiz} onClick={async () => {
              if (!result?.quiz?.length) return; setPdfBusy(s => ({ ...s, quiz: true }));
              try { const jsPDF = await ensureJsPDF(); const doc = new jsPDF(); addHeader(doc); let y = 32; doc.setFontSize(13); y = lineWrap(doc, `Title: ${result.title || "Untitled"}`, 15, y, 180); doc.setFontSize(12);
                for (let i = 0; i < result.quiz.length; i++) { const q = result.quiz[i]; y = lineWrap(doc, `Q${i + 1}. ${q.question}`, 15, y, 180); for (let oi = 0; oi < q.options.length; oi++) { const prefix = String.fromCharCode(65 + oi); y = lineWrap(doc, `${prefix}) ${q.options[oi]}`, 20, y, 170); }
                  const correctLetter = String.fromCharCode(65 + (q.answer_index ?? 0)); y = lineWrap(doc, `Correct: ${correctLetter}) ${q.options[q.answer_index]}`, 20, y + 2, 170);
                  if (showExplanations && q.explanation) { y = lineWrap(doc, `Why: ${q.explanation}`, 20, y + 2, 170); }
                  y += 4; }
                addFooter(doc); doc.save("skriptio-quiz.pdf"); } finally { setPdfBusy(s => ({ ...s, quiz: false })); }
            }} aria-busy={pdfBusy.quiz}>
              {pdfBusy.quiz ? <><Loader2 className="mr-2 h-3.5 w-3.5 animate-spin"/> Generating...</> : 'Download Quiz PDF'}
            </Button>
            <Button variant="outline" disabled={!result?.flashcards?.length || pdfBusy.cards} onClick={async () => {
              if (!result?.flashcards?.length) return; setPdfBusy(s => ({ ...s, cards: true }));
              try { const jsPDF = await ensureJsPDF(); const doc = new jsPDF(); addHeader(doc); let y = 32; doc.setFontSize(13); y = lineWrap(doc, `Title: ${result.title || "Untitled"}`, 15, y, 180); doc.setFontSize(12);
                result.flashcards.forEach((c, idx) => { y = lineWrap(doc, `Card ${idx + 1}:`, 15, y, 180); y = lineWrap(doc, `Q: ${c.front}`, 20, y, 170); y = lineWrap(doc, `A: ${c.back}`, 20, y, 170); y += 4; }); addFooter(doc); doc.save("skriptio-flashcards.pdf"); } finally { setPdfBusy(s => ({ ...s, cards: false })); }
            }} aria-busy={pdfBusy.cards}>
              {pdfBusy.cards ? <><Loader2 className="mr-2 h-3.5 w-3.5 animate-spin"/> Generating...</> : 'Download Flashcards PDF'}
            </Button>
            <Button variant="outline" disabled={!theory?.length || pdfBusy.theory} onClick={async () => {
              if (!theory?.length) return; setPdfBusy(s => ({ ...s, theory: true }));
              try { const jsPDF = await ensureJsPDF(); const doc = new jsPDF(); addHeader(doc); let y = 32; doc.setFontSize(13); y = lineWrap(doc, `Title: ${result?.title || "Untitled"}`, 15, y, 180); doc.setFontSize(12);
                theory.forEach((t, idx) => { y = lineWrap(doc, `Q${idx + 1}: ${t}`, 15, y, 180); y += 4; }); addFooter(doc); doc.save("skriptio-theory.pdf"); } finally { setPdfBusy(s => ({ ...s, theory: false })); }
            }} aria-busy={pdfBusy.theory}>
              {pdfBusy.theory ? <><Loader2 className="mr-2 h-3.5 w-3.5 animate-spin"/> Generating...</> : 'Download Theory PDF'}
            </Button>
            <Button variant="outline" disabled={!result?.plan?.length || pdfBusy.plan} onClick={async () => {
              if (!result?.plan?.length) return; setPdfBusy(s => ({ ...s, plan: true }));
              try { const jsPDF = await ensureJsPDF(); const doc = new jsPDF(); addHeader(doc); let y = 32; doc.setFontSize(13); y = lineWrap(doc, `Title: ${result.title || "Untitled"}`, 15, y, 180); doc.setFontSize(12);
                result.plan.forEach((d) => { y = lineWrap(doc, `${d.title}`, 15, y, 180); d.objectives.forEach((o) => { y = lineWrap(doc, `• ${o}`, 20, y, 170); }); y += 4; }); addFooter(doc); doc.save("skriptio-plan.pdf"); } finally { setPdfBusy(s => ({ ...s, plan: false })); }
            }} aria-busy={pdfBusy.plan}>
